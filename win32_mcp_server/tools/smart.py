...
try:
    ...
    def _fuzzy_match(query: str, candidate: str) -> int:
        return int(_rf_fuzz.partial_ratio(query.lower(), candidate.lower()))
except ImportError:
...
def _find_text_matches(
    ...
    for key, words in lines.items():
        ...
        if not exact:
            score = _fuzzy_match(search_text, line_text)
            if score >= threshold:
    ...
    if not matches and not exact:
        for r in ocr_results:
            score = _fuzzy_match(search_text, r["text"])
            if score >= threshold:
                bbox = {
                    "text": r["text"],
                    "x": r["x"],
...
def _extract_matching_words(
    words: list[dict], line_text: str, start_idx: int, match_len: int,
) -> list[dict]:
...
def _merge_bounding_boxes(words: list[dict]) -> dict:
...
async def find_text_impl(
    ...
    if window_title:
        ...
        # Capture window region — handle windows extending off-screen
        left = max(0, win.left)
        top = max(0, win.top)
    ...
    ocr_results = await ocr_structured_impl(
        img, lang=lang, preprocess=preprocess,
    )
...
@registry.register("find_text_on_screen", "Find all occurrences of text on screen with coordinates", {
    "type": "object",
    "properties": {
...
async def handle_find_text_on_screen(arguments: dict):
    text = arguments["text"]
...
@registry.register("click_text", "Find text on screen and click on it", {
    "type": "object",
    "properties": {
...
async def handle_click_text(arguments: dict):
    text = arguments["text"]
...
@registry.register("wait_for_text", "Wait until specific text appears on screen", {
    "type": "object",
    "properties": {
...
async def handle_wait_for_text(arguments: dict):
    text = arguments["text"]
    ...
    while True:
        ...
        try:
            ...
            pass  # OCR or capture might fail transiently

        elapsed = time.monotonic() - start
        if elapsed >= timeout:
            raise ToolError(
                f"Text '{text}' did not appear within {timeout}s ({attempts} checks)",
                suggestion="Increase timeout_seconds, check if the correct window is open, or use capture_screen to see what's visible.",
            )

        await asyncio.sleep(interval)


@registry.register("assert_text_visible", "Assert that text is or is not visible on screen (for testing)", {
    "type": "object",
    "properties": {
...
async def handle_assert_text_visible(arguments: dict):
    text = arguments["text"]
    ...
    if should_exist and not found:
        ...
        return {
            ...
            "match_count": len(matches),
            "found_at": [{"x": m["screen_center_x"], "y": m["screen_center_y"], "text": m["text"]} for m in matches[:3]],
        }
    else:
        return {
            "passed": True,
...
@registry.register("fill_field", "Click a labeled input field and type a value", {
    "type": "object",
    "properties": {
        "label_text": {
            "type": "string",
...
async def handle_fill_field(arguments: dict):
    ...
    if not value.isascii():
        ...
        await asyncio.to_thread(
            pyautogui.write, value, interval=config.automation.type_interval,
        )
...
@registry.register("get_window_snapshot", "Capture complete window state: screenshot + OCR text with positions", {
    "type": "object",
    "properties": {
...
async def handle_get_window_snapshot(arguments: dict):
    ...
    if include_ocr:
        ...
        for r in ocr_results:
            r["screen_x"] = r["x"] + win_info["x"]
            r["screen_y"] = r["y"] + win_info["y"]
...
@registry.register("right_click_menu", "Right-click at a position and OCR the context menu", {
    "type": "object",
    "properties": {
...
async def handle_right_click_menu(arguments: dict):
    ...
    try:
        ...
        # Fall back to full screen if region fails
        img = await capture_screen_impl()
...
@registry.register("execute_sequence", "Execute multiple tools in sequence without round-trip overhead", {
    "type": "object",
    "properties": {
        "steps": {
            "type": "array",
...
async def handle_execute_sequence(arguments: dict):
