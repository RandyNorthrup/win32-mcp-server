...
@registry.register("clipboard_copy", "Copy text to the system clipboard", {
    "type": "object",
    "properties": {
...
async def handle_clipboard_copy(arguments: dict):
    text = arguments["text"]
    pyperclip.copy(text)
    return [TextContent(type="text", text=f"Copied {len(text)} characters to clipboard")]


@registry.register("clipboard_paste", "Read the current clipboard text contents", {
    "type": "object",
    "properties": {},
})
async def handle_clipboard_paste(arguments: dict):
    try:
        text = pyperclip.paste()
    except Exception as exc:
        raise ToolError(f"Failed to read clipboard: {exc}")
